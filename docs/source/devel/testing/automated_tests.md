...
- Slow tests (marked `slow`) require the `--longtests` flag
...
Writing a pipeline test requires the following inputs in the
...
The coverage report is saved to `htmlcov/index.html`.

### Combining coverage from multiple runs

Merge `.coverage.*` data files generated by parallel or separate runs:
