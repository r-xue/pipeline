...
def get_plot(plots, prefix, datatype, field, spw, i, colname, moment=None):
    try:
        if moment is not None:
            return plots[prefix][datatype][field][spw][i][colname][moment]
        else:
            for m in ('mean', 'mom8', 'mom0', 'N/A'):
                if m in plots[prefix][datatype][field][spw][i][colname]:
                    return plots[prefix][datatype][field][spw][i][colname][m]
            return None
    except KeyError:
        return None
%>
...
<table class="table table-striped">
    ...
    <tbody>
        ...
        % for i in sorted(plots_dict[prefix][datatype][field][spw].keys())[::-1]:
            % for colorder in colorders:
                ...
                <td>
                    <% plot = get_plot(plots_dict, prefix, datatype, field, spw, i, colname, moment) %>
        ...
        <tr>
            ...
            % for colname in colnames:
                <td>
                    ...
                    <% 
                    lastiter = sorted(plots_dict[prefix][datatype][field][spw].keys())[-1]
                    plot = get_plot(plots_dict, prefix, datatype, field, spw, lastiter, colname)
                    %>
                    % if plot is not None:
                        <div class="thumbnail">
                            <a data-fancybox
