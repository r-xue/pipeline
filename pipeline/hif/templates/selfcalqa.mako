...
<table class="table table-bordered">
  ...
  if any(v is not None for v in fracflag_plots.values()):
    caption_items.append('<li> Any lower-panel plot of each EB shows the fraction of solutions flagged for each antenna (blue circle) as a function of baseline length, along with a smoothed, continues version of this distribution (black), and the first (green) and second (red) derivatives of this distribution. Long baselines are defined as the place where the flagging fraction begins to significantly increase (horizontal dashed line), as measured by the locations of significant peaks in the acceleration, and the threshold for passing through antennas with no calibration is defined by the intersection of the long baseline threshold with the smoothed, flagged fraction curve (horizontal dashed line). Antennas with flagged fractions above this threshold will have no calibrations applied but also will not be flagged, i.e. their solutions are set to 1.0+0j. </li>')
  if caption_items:
