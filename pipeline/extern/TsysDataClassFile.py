...
# Importing packages
import sys, os, glob, numpy as np, re,  warnings, itertools, bz2, pickle

sys.path.append(".")#/opt/pipelinedriver/2022AUG/casa/casa-6.4.1-12-pipeline-2022.2.0.64/pipeline/pipeline/extern/")
...
def ATMtrans(msfile, scans, spws, ifld=None, verbose=False):
    ...
    myscantimes = [np.median(msmd.timesforscan(iscan)) for iscan in scans] # MJD in seconds
    ...
    for iscan, pwv, P, H, T, airmass in zip(scans, wf['pwv'](_st),wf['pressure'](_st),wf['humidity'](_st),wf['temperature'](_st), airmasses):
        T += 273.15 # Kelvin
        ...
        for ispw in spws:
            ...
            if band in [9,10]:
                nbands = 2
                freqs_SB, chansep_SB, center_SB, width_SB = getImageSBFreqs(msfile,ispw)
            ...
            if band in [9, 10]:
                ...
                results[f'{ispw}_{iscan}'] = (np.array(transmission)+np.array(transmission_SB))/2 # v2.5
            else:
...
def weather_function(msfile):    
    ...
    try:
        ...
        print('''This measurement set does not have or has problems with ASDM_CALWVR table. Try the option
...
in importasdm''')
    ...
    if len(pwv)<=0:
        try:
            ...
            pwvtime = tb.getcol('startValidTime')  # mjdsec
            antenna = tb.getcol('antennaName')
            pwv = tb.getcol('water')[0]  # There seem to be 2 identical entries per row, so take first one.
            tb.close()
            conditions['pwv'] = lambda tt: np.interp(tt,pwvtime,pwv)
        except RuntimeError:
            print('''This measurement set does not have an ASDM_CALATMOSPHERE table. Try the option
asis='SBSummary ExecBlock Annotation Antenna Station Receiver Source CalAtmosphere CalWVR CalPointing' 
in importasdm''')
            raise RuntimeError()
    ...
    try:
        ...
        print('''This measurement set does not have an ASDM_STATION table. Try the option
asis='SBSummary ExecBlock Annotation Antenna Station Receiver Source CalAtmosphere CalWVR CalPointing' 
in importasdm''')
        raise RuntimeError()
    ...
    pgt0 = pressure>0
    try:
        assert np.any(pgt0), "weather_function: No valid weather data! Returning default conditions."
    except AssertionError:    
        conditions['pressure'] = lambda tt: 563.0
        conditions['temperature'] = lambda tt: 0.0 # in deg C
        conditions['humidity'] = lambda tt: 20.0 
    ...
    data_weather = {'pressure':pressure, 'humidity':relativeHumidity, 
                    'temperature':temperature, 'mjdsec':mjdsec}
    #quantile = np.quantile(np.diff(mjdsec),(1-1/len(mjdsec))/nstations) 
    
    split_idx = np.split(np.arange(mjdsec.shape[0]), np.where(np.diff(mjdsec) > WEATHER_DATA_GROUPING)[0]+1)
    ...
    for k in (set(data_weather.keys())-set(['mjdsec'])):
        ...
        conditions[k] = lambda tt, times=data_weather['mjdsec'], vals=data_weather[k]: np.interp(tt,times,vals)
...
def chanfreq_records_to_functions(chan_records):
        ch_frq = chan_records
        freqMHz = dict(); chan = dict(); numchannels = dict();
        for recn in ch_frq.keys():
            chf = ch_frq[recn][:,0]
            spw = int(recn.replace('r',''))-1
            f0,f1,nchan = [chf[0],chf[-1],chf.shape[0]]
            freqMHz[spw] = lambda ch,f0=f0,f1=f1, nchan=nchan: (f0+ch*(f1-f0)/nchan)/1e6
            chan[spw] = lambda fMHz,f0=f0,f1=f1, nchan=nchan: (fMHz*1e6-f0)/(f1-f0)*nchan
            #chansizeMHz[spw] = ((f1-f0)/1.0e6/nchan)
            numchannels[spw] = nchan
        return freqMHz, chan, numchannels

class TsysData(object):
    def __init__(self, tsystable=None, oldobject=None, load_pickle=False, single_polarization=False):
        self.tsystable = tsystable
        if tsystable is None: return None
        ...
        if load_pickle:
            ...
            try:
                with bz2.BZ2File(file, 'rb') as reader:
                    ...
                    assert 'VERSION' in data_dict, "No version information in the picke file!"
                    assert data_dict['VERSION'] >= VERSION, f"Version of pickle is {data_dict['VERSION']} < {VERSION}, which is the version of the script."
                ...
                freqMHz, chan, numchannels = chanfreq_records_to_functions(data_dict['ch_frq'])
                self.msspec = dict(((spw,(freqMHz[spw],chan[spw],numchannels[spw])) for spw in freqMHz.keys())) # v2.4
                return
            except OSError:
        ...
        chan = dict()
        numchannels = dict()
        assert tb.open(f'{tsystable}/SPECTRAL_WINDOW'), f"Could not open {tsystable}/SPECTRAL_WINDOW"
        ...
        ant1,scans,spws, fields = tb.getcol('ANTENNA1'),tb.getcol('SCAN_NUMBER'),tb.getcol('SPECTRAL_WINDOW_ID'), tb.getcol('FIELD_ID')
        ...
        self.tsysmap = TsysData.tsysmap(vis=msfile, tsystable=tsystable, msmdtool_instance = msmd) # v2.4
        auxi = dict()
        ...
        for scan in set(list(scans)):
            ...
            fieldnumbers_for_fieldname = msmd.fieldsforname(msmd.fieldnames()[msmd.fieldsforscan(scan)[0]])
            all_intents = []
        ...
        atmos_transmission = ATMtrans(msfile=msfile,scans=set(list(scans)), spws=set(list(spws)), verbose=False)
        assert np.all(np.array([np.all(_<=1) for _ in atmos_transmission.values()])), "Not all transmission curves <=1! Something is wrong!"
        ...
        for recn,antenna,scan,spw in zip(range(1,nrows+1),ant1,scans,spws):        
            ...
            spec = fparam[f'r{recn}'][:,:,0] # np.mean(fparam[f'r{recn}'][:,:,0], axis=0) # here the polarization average occurs.
    ...
    def tsysmap(vis,tsystable, msmdtool_instance,intent='OBSERVE_TARGET#ON_SOURCE'):  # v2.4, this function
        ...
        if np.mean(value) > -1:
            ...
            return(dict(((sspw,tspw) for sspw,tspw in zip(list(scienceSpws),list(_tsysmap[scienceSpws])))))
            #return(scienceSpws, _tsysmap)
        else:    
            #mymsmd.close()
            raise IntentException("%s not found in this dataset. Available intents: " % (intent), allIntents)
    ...
    def get_valid_antennae(self):
        ...
        exclude = set()
        ##
        [antennas,spws,scans,fields,flags, intents, tsyss] = [self.tsysdata[self.tsysfields.index(k)] 
                                            for k in ('antenna','spw','scan','field','flags','intent','tsys')]
        valid = set(itertools.product(set(antennas),set(spws),set(scans), set(fields)))
        for antenna,spw,scan,field in valid:
            selection = np.nonzero((antennas==antenna) * (spws==spw) * (scans==scan) * (fields==field))[0]
            if len(selection)<=0: continue
            assert len(selection) == 1, f"Selection antenna={antenna},spw={spw},scan={scan},field={field} has more than 1 match!"
            selection = selection[0]
            flag = flags[selection]
            if np.all(flag):
                exclude = exclude.union(set([f'{spw}_{field}_{scan}_{antenna}_{intent}']))
            else:
    ...
    def _combine_difference(minuend_dict,subtrahend_dict):
        ...
        k = minuend_keys[0]
        m = re.match(r'(?P<spw>\d+)_(?P<field>\d+)_(?P<scan>\d+)_(?P<antenna>.+)_(?P<intent>.+)',k)
        ...
        for mi,su in itertools.product(minuend_keys,subtrahend_keys):
            m = re.match(rf'{spw}_{field}_(?P<scan>\d+)_(?P<antenna>.+)_(?P<intent>.+)',mi)
            ...
            if not antenna_mi==antenna_su: continue
            differences[f'{spw}_{scan_mi}-{scan_su}_{antenna_mi}'] = minuend_dict[mi]-subtrahend_dict[su]
    ...
    def filter_statistics_and_dif(self, statistics,  remove_n_extreme=1, polarizations=None):
        ...
        partitioned_keys_by_field = TsysData._partition_by_equivalence_class(valid_keys,fequiv)
        ...
        for keyset_s in partitioned_keys_source:
            # print(keyset_s)
            m = re.match(r'(?P<spw>\d+)_(?P<field>\d+)_(?P<scan>\d+)_(?P<antenna>.+)_(?P<intent>.+)',next(iter(keyset_s)))
            spw,field,intent_sou = m.group('spw','field','intent')
            sourcedict = dict([(k,valid[k]) for k in keyset_s])
            bpdict_spw = dict([(k,v) for k,v in bpdict.items() if re.match(fr'^{spw}_.*',k)])
            
            diff = TsysData._combine_difference(minuend_dict=sourcedict, subtrahend_dict=bpdict_spw)

            m = re.match(r'(?P<spw>\d+)_(?P<field>\d+)_(?P<scan>\d+)_(?P<antenna>.+)_(?P<intent>.+)',next(iter(bpdict_spw.keys())))
            field_bp, scan_bp = m.group('field','scan')
            diffkeyslist = list(diff.keys()).copy()
            # print(spw,field,intent_sou, diffkeyslist)

            m = re.match(r'(?P<spw>\d+)_(?P<scan_sou>\d+)-(?P<scan_bp>\d+)_(?P<antenna>.+)',diffkeyslist[0])
            ...
            auxiliar = (auxi_dif-np.nanmean(auxi_dif,axis=(1,2))[:,np.newaxis,np.newaxis]) 
            order_max = np.argsort(np.nanmax(auxiliar,axis=(1,2))) # np.argsort(np.nanmax(auxi_dif-np.nanmean(auxi_dif,axis=1)[:,np.newaxis],axis=1))
            order_min = np.argsort(np.nanmin(auxiliar,axis=(1,2))) # np.argsort(np.nanmin(auxi_dif-np.nanmean(auxi_dif,axis=1)[:,np.newaxis],axis=1))
            selection = np.array(list(set(range(auxi_dif.shape[0])) 
            ...
            auxi_dif_antenna = np.r_[[re.match(rf'^{spw}_.*_(?P<antenna>.+)$',k).group('antenna') for k in diffkeyslist]]
            ...
            auxi_sou = np.c_[[valid[re.sub(fr'^{spw}_(\d+)-{scan_bp}_([^_]+)$',rf'{spw}_{field}_\1_\2_{intent_sou}',k)] for k in diffkeyslist]]
            auxi_bp = np.c_[[bpdict[re.sub(fr'^{spw}_(\d+)-{scan_bp}_([^_]+)$',rf'{spw}_{field_bp}_{scan_bp}_\2_bandpass',k)] for k in diffkeyslist]]
