...
def evalPerAntBP_Platform(pickle_file, inputpath, vis, caltable):
    LOG.info('Doing Platforming evaluation')
    if glob.glob('*platform.txt'):
        os.system("rm -rf *platform.txt")  # TODO: Update these to overwrite if present rather than rm -rf 

    if glob.glob('*platform_value.txt'):
        os.system("rm -rf *platform_value.txt")  # TODO: Update these to overwrite if present rather than rm -rf
    
    if glob.glob('*flagging.txt'):
        os.system("rm -rf *flagging.txt")  # TODO: Update these to overwrite if present rather than rm -rf
...
def bandpass_platforming(ms: MeasurementSet, caltable, inputpath='.', outputpath='./bandpass_platforming_qa'):
   ...
   # FIXME: kberry: switch to using the _one_ passed in ms and caltable here - update to better structure next.
   #mytablelist=glob.glob('uid*bandpass.s*channel.solintinf.bcal.tbl')
   mytablelist = [caltable]
   myworkdir = glob.glob(inputpath)
   inputpath = os.path.abspath(inputpath)
   LOG.info(f"mytablelist: {mytablelist}")
   LOG.info(f"myworkdir: {myworkdir}")
...
#   os.chdir(outdir)
#   LOG.info("changed dir to:", outdir)
...
# FIXME: eliminate this code unless it needs to be able to be tested outside of a PL run 
# (though at this point pipeline would *absolutely* be a dependency.)
#
# def main(argv):
