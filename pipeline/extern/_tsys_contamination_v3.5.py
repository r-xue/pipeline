...
# Importing packages
import os, glob, numpy as np, re, matplotlib.pyplot as plt, warnings, xml.etree.ElementTree as ET, itertools, bz2, pickle, random
from casarecipes.almahelpers import tsysspwmap # v2.4
#from math import *
...
if True:
    ...
    CO_LINES_MHZ = {'C18O_1-0':109782.17340, '13CO_1-0':110201.35430, 'CO_1-0':115271.20180, 
        'C18O_2-1':219560.35410, '13CO_2-1':220398.68420, 'CO_2-1':230538.00000, 'C18O_3-2':329330.55250, 
        '13CO_3-2':330587.96530, 'CO_3-2':345795.98990, 'C18O_4-3':439088.76580, '13CO_4-3':440765.17350, 
        'CO_4-3':461040.76820, 'C18O_6-5':658553.27820, '13CO_6-5':661067.27660, 'CO_6-5':691473.07630, 
        'CO_7-6':806651.80600, 'C18O_8-7':877921.95530, '13CO_8-7':881272.80800, 'CO_8-7':921799.70000,}
    LINES_12C16O = np.array([v for k,v in CO_LINES_MHZ.items() if re.match(r'CO_\d\-\d',k)])
...
#preq232 @ jupyterif True:
    def gline_and_baseline(pars,x, deg=1):
        ...
        if deg==1:
            return pars[0]*np.exp(-4*np.log(4)*(x-pars[1])**2/(np.finfo(np.float64).eps+pars[2]**2))+pars[3]+pars[4]*x
        if deg==2:
            return pars[0]*np.exp(-4*np.log(4)*(x-pars[1])**2/(np.finfo(np.float64).eps+pars[2]**2))+pars[3]+pars[4]*x+pars[5]*x**2

    def gaussian_line_fit(x,y, sigma=1):
        #(peak,x0,fwhm)=pars    
        xi2 = lambda pars: np.sum(no_nans(y-gline_and_baseline(pars,x))**2/sigma)
        chan_max = np.argmax(y)   
        pars0 = [y[chan_max]-np.min(y),chan_max+x[0],min(np.sum(y>(y[chan_max]+np.min(y))/2),len(x)/2),np.min(y),0] # 4 channels initial parameter fwhm
        
        #print(pars0)
        degree = 1
        lb = [-np.inf for _ in range(len(pars0))];ub = [np.inf for _ in range(len(pars0))];
        lb[0] = 0; ub[0] = np.nanmax(y)-np.nanmin(y)
        lb[1] = np.min(x); ub[1] = np.max(x)
        ...
        if res['fun']/len(x)>15:
            ...
            for deg in [1,2]:
                ...
                xi2 = lambda pars: np.sum(no_nans(y-gline_and_baseline(pars,x, deg=deg))**2)
                for fwhm_ratio in [3,5,7]:
                    pars0[2] = min(np.sum(y>(y[chan_max]+np.min(y))/2),len(x)/fwhm_ratio)
                    resaux = minimize(xi2,pars0+([] if deg==1 else [0]),bounds=Bounds(lb=lb,ub=ub))
                    # print(f"--{resaux['fun']}")
                    if resaux['fun']/len(x)<2 or resaux['fun']<=res['fun']-0.5*len(x):
                        res = resaux; degree=deg
    ...
    def width_fit_filter(data,peaks=None, fwhm_limit=None, window=7):
        ...
        d = np.r_[np.ones(half_window_size)*np.nan,data,np.ones(half_window_size)*np.nan]    
        #widths = []
        filtered_peaks = []
        for p in peaks+half_window_size:
            gpars, gline = gaussian_line_fit(x=np.arange(-half_window_size,half_window_size+1),
    ...
    def find_repeated_peaks(data1,data2,prominence, 
        prominence_limit=5, # v3.3
        difference_tolerance_ratio=0.5, width=None, distance_from_peak=1): # v2.5
        ...
        distance = lambda var: np.sum(no_nans((var[0]*x**3+var[1]*x**2+var[2]*x+var[3]-((data1+data2)/2))**2))
        ...
        for _p1,_pp1 in zip(p1,prominence_dictionary1['prominences']):
            tol = int(np.floor(np.sqrt(np.log(_pp1/prominence)/np.log(2))+.5))
            # print(_p1,_pp1, tol)
            _p2 = set(list(np.r_[[_p1+delta for delta in range(-tol,tol+1)]].flatten())).intersection(set(list(p2)))
            if len(_p2)==0: continue
            repeated_peak_channels1.append(_p1)
            if len(_p2)==1:
                repeated_peak_channels2.append(next(iter(_p2)))
            else:
                _p2 = list(_p2)
                _pp2 = prominence_dictionary2['prominences'][np.isin(p2,np.array(list(_p2)),assume_unique=True)]
        ...
        for rp1, rp2 in zip(repeated_peak_channels1,repeated_peak_channels2):
            width_rp1 =  peak_widths(data1,p1[p1==rp1],rel_height=distance_from_peak/prominence_dictionary1['prominences'][p1==rp1][0])[0][0] 
            width_rp2 =  peak_widths(data2,p2[p2==rp2],rel_height=distance_from_peak/prominence_dictionary2['prominences'][p2==rp2][0])[0][0]
            if not(width_rp1<=width and width_rp2<=width): continue
            if difference_tolerance_ratio < 1:
                ...
                comparison_ratio = np.abs((_prom1-_prom2)/(_prom1+_prom2)) # change formula in v3.3
                
                if comparison_ratio>=difference_tolerance_ratio: continue
                if _prom1>prominence_limit and _prom2>prominence_limit:
                    # print(rp1,_prom1)
                    auxi1.append(rp1)
                    dd1['prominences'].append(_prom1) # v3.4
                    dd1['left_bases'].append(prominence_dictionary1['left_bases'][p1==rp1]) # v3.4
                    dd1['right_bases'].append(prominence_dictionary1['right_bases'][p1==rp1])
                    auxi2.append(rp2)
                    dd2['prominences'].append(_prom2) # v3.4
                    dd2['left_bases'].append(prominence_dictionary2['left_bases'][p2==rp2]) # v3.4
                    dd2['right_bases'].append(prominence_dictionary2['right_bases'][p2==rp2])
    ...
    def model_break(model, peaks,window_length = 5):   # v3.5
        ...
        if len(peaks)<=0: return xp , np.interp(chans,xp[0],model[xp[0]])
        window_length+=(1-window_length%2)
        (atm_1, atm_2) = (savgol_filter(model,window_length=window_length, deriv=dd,polyorder=3) for dd in(1,2))
        if model[0]==np.max(model) and atm_1[0]==np.min(atm_1) and atm_2[0]==np.max(atm_2):
            peaks = np.r_[0,peaks]
        if model[-1]==np.max(model) and atm_1[-1]==np.min(atm_1) and atm_2[-1]==np.max(atm_2):
            peaks = np.r_[peaks,nchan-1]
        # assert False, peaks
        if len(peaks)<=1: return xp , lower_hug(model)

        _ = find_peaks(model,prominence=0) # v2.5
        _auxp,_auxpp = find_peaks(model,prominence=np.mean(np.sort(_[1]['prominences'])[-len(_[0]):-len(_[0])+1])) # v2.5 # v3.2 
        crossings = np.array(list(set(list(np.r_[_auxpp['left_bases'],_auxpp['right_bases']])))) # v2.5
        ...
        xp = np.array(np.r_[[] if 0 in peaks else 0,crossings,int([] if nchan-1 in peaks else nchan-1)],dtype=int)
        # print(f'xp={xp}')
    ...
    def lower_hug(y):        
        ...
        for v in set(list(hull.simplices.flatten())): 
            nodes[v] = list(set(list((hull.simplices[np.sum(hull.simplices==v,axis=1,dtype=bool)].flatten())))-set([v]))
        lower = [0]
        node0 = 0
        ...
        while node1<len(y)-1:
            ...
            assert len(_)==1, f"lower_hug: bad node construction.\nnodes={nodes}\nnode0={node0}\nnode1={node1}"
            node0 = node1
            node1 = next(iter(_))            
            lower.append(node1)
        # assert False, lower
    ...
    def modified_atmospheric_profile(model, baseline, degree_baseline, scales, coefficients, segments, x_coordinates=None, split=False):   
        ...
        for nseg, seg_resc in enumerate(list(segments_resc)):
            a = coefficients[2*nseg*n_scales+degree_baseline+2:2*nseg*n_scales+degree_baseline+2+n_scales]
            b = coefficients[2*nseg*n_scales+degree_baseline+2+n_scales:2*nseg*n_scales+degree_baseline+2+2*n_scales]
            # print(f"---{a}--{b}")
            if n_scales>0:
                for ii,scale in enumerate(scales):
                    c0,c1 = segments[nseg,0],segments[nseg,1]
                    if np.sum(model[c0:c1]**2)>0:
                        modified[c0:c1] += (a[ii]*exp_rescale(data=model_mod[c0:c1],scale=scale) 
    ...
    def cross_model(data_a, data_b, verbose=False, asym_factor=1):
        ...
        dif2 = lambda pp: np.sum(no_nans(np.array([_ if _ >=0 else asym_factor*_ for _ in data_a-model(pp)]))**2)
    ...
    def max_without_outliers(x):
        ...
        while np.any(outliers):
            ...
            if sigma*len(_x)<=0:
                break
            outliers  = np.abs(_x-med)/sigma > np.abs(scipy.stats.norm.ppf(0.5/len(no_nans(_x))))
            # print(med,sigma,outliers)
            _x = _x[np.logical_not(outliers)]
        return np.nanmax(_x)


    def positive_line_intervals(normalized_data, detection_limit=7, base_detection=2,edge = 0.025, verbose=False): 
        ...
        peaks0, peaks_dict0 = find_peaks((_norm)*(_norm>base_detection),height= detection_limit,prominence= detection_limit)   
        peaks_dict0['peak_heights']+=base_detection
        
        if verbose:
            print(f"positive_line_intervals: peaks0, peaks_dict0 = {peaks0}, {peaks_dict0}")
            plt.close();plt.plot(normalized_data); 
            ...
            plt.plot(detection_limit*np.ones(len(normalized_data)),linestyle='dotted');
            plt.title(f'{peaks0}')
            plt.show(); input()
        
        lims = np.sort(np.array(list(set(list(np.r_[peaks_dict0['left_bases'],peaks_dict0['right_bases']]))))) # v3.3
        intervals = np.array([[np.max(lims[lims<=p]),np.min(lims[lims>=p])] for p in peaks0]) #merge_intervals(_) # v3.3
        ...
        for interval in intervals:
            ...
            inside_peaks = np.c_[peaks0[_],peaks_dict0['peak_heights'][_],peaks_dict0['prominences'][_]]
            inside_peaks = inside_peaks[(-inside_peaks[:,1]).argsort()]
            print(f"positive_line_intervals:\n\t\tinside_peaks={inside_peaks}");#input()
        ...
        return (np.array(peaks, dtype=int), # v2.3
                np.array(peaks_intervals, dtype=int).reshape(len(peaks),2), 
                np.array(heights))

    
    def refitting_intervals_bp_to_source(source, bp, sigma, normalized_data, detection_limit, edge=0.025,base_detection=2, verbose=True):
        peaks, intervals, heights = positive_line_intervals(normalized_data=normalized_data, edge=edge, verbose=False,
                                                            detection_limit = detection_limit,base_detection=base_detection)
        
        if verbose: print(f"refitting_intervals_bp_to_source:\n\t--First version: {intervals}"); #input()
        final_peaks, final_intervals = np.array([]),np.array([])
        for interval in intervals:
            shift = shift_bp_to_source(interval=interval,bp=bp,source=source, sigma=sigma,verbose=verbose)
            # print(intervals, shift)
            interval = np.array(interval,dtype=int)
            ch0,ch1 = interval[0], interval[1]
            peaks1, intervals1, heights1 = positive_line_intervals(normalized_data=((source-bp-shift)/sigma)[ch0:ch1+1], 
                                                                    edge=0, 
                                                                    detection_limit = detection_limit,base_detection=base_detection, 
                                                                    verbose=False)
            # plt.plot(((source-bp-shift)/sigma)[ch0:ch1+1]); plt.show();input()
            
            peaks1+=ch0; intervals1+=ch0;
            if verbose: print(f"refitting_intervals_bp_to_source:\n\t--Second version: {intervals1}"); # input()
            final_peaks = np.r_[final_peaks, peaks1]   
            final_intervals = intervals1 if final_intervals.shape[0]==0 else np.r_[final_intervals, intervals1]
    ...
    def below_fit(data,model,iterations, clip, start_asym_factor=2):    
        ...
        for k in range(10): # v3.5
            ...
            for i in range(iterations):
                ...
                dif = lambda pp: np.sum(no_nans(np.abs(np.array([_ if _ >=0 else asym_factor*_ for _ in _data-model-pp]))))
                ...
                #print(f"clip={clip}");
                if False:
                    plt.plot(_data); plt.plot(model+res['x'][0]);input()
    ...
    def shift_bp_to_source(interval,source, bp, sigma, clip_snr=4, iterations=2,asym_factor=2,verbose=False): # v3.5
        nchan = len(source)
        chans = np.arange(0,nchan)
        interval = np.array(interval,dtype=int)
        delta = search_expanded_interval(data = bp, interval=interval, sigma=np.nanmedian(sigma), limit_sigma=1, verbose=True)
        if delta == 0: delta = (interval[1]-interval[0])/2
        ch0,ch1 = max(0,int(interval[0]-delta/2)), int(min(interval[1]+delta/2,nchan-1))
        if verbose: print(f'shift_bp_to_source {ch0},{ch1}')
       
        shift = below_fit(data=(source)[ch0:ch1+1],model=(bp)[ch0:ch1+1],iterations=iterations, 
                            clip=clip_snr*np.nanmedian(sigma),start_asym_factor=1)#asym_factor)
        if False: 
            plt.plot(source[ch0:ch1+1]);plt.plot(shift+bp[ch0:ch1+1]);input();
        if verbose: print(f'shift={shift}')
        return shift

    def search_expanded_interval(data, interval, sigma, limit_sigma=2,verbose=False):
        if verbose: print(f"---initial interval={interval}")
        ch0,ch1 = int(interval[0]), int(interval[1])
        delta0 = ch1-ch0
        nchan = data.shape[0]
        delta_return = 0
        for delta in range(int(min(2*interval[0]/3,2*(nchan-interval[1])/3))+1):
            _ch0,_ch1 = int(ch0-delta/2), int(ch1+delta/2)
            x = np.linspace(_ch0,_ch1,_ch1-_ch0+1)
            y = data[_ch0:_ch1+1]
            # print(x.shape,y.shape)
            m,n = np.polyfit(y=y,x=x,deg=1)
            reduced_chi_square = np.sum(no_nans(m*x+n-y)**2)/sigma**2/(_ch1-_ch0+1)       
            if reduced_chi_square>1+limit_sigma*np.sqrt(2/(_ch1-_ch0+1)):
                if verbose: print(f"---redchi-sq = {reduced_chi_square}, delta_return = {delta_return}")
                return delta_return
            else:
                delta_return = delta
        if verbose: print(f"---redchi-sq = {reduced_chi_square}, delta_return = {delta_return}")
    ...
    def basic_atm_asymmetric_fit(data,transmission,  iterations, clip,asym_factor=2):
        ...
        model = lambda pp: pp[0]*(1-transmission)+pp[1]*xvec+pp[2] # include base in v3.3
        _data = data.copy()    
        for i in range(iterations):
            dif2 = lambda pp: np.sum(no_nans(np.abs(np.array([_ if _ >=0 else asym_factor*_ for _ in _data-model(pp)]))))
            meantsys = np.nanmean(data)
            pars0 = [meantsys/np.nanmean(1-transmission),0,0]
            _ = np.abs(meantsys/np.nanmean(1-transmission))
            # print(pars0)
            res = minimize(dif2,pars0, bounds=[(-10*_,10*_),(None,None),(None,None)])
            _data[np.abs(_data-model(res['x'])) > clip*np.nanmedian(np.abs(_data-model(res['x'])))]
        return res['x']
    ...
    def smoother_sigma(v,iterations=2, clip_level=7, stype='sg', verbose=False):   
        ...
        for i in range(iterations):
            ...
            if verbose: print(f'smoother_sigma:\n\tsub={sub}\n\tsigma={sigma}') 
    ...
    def smoother(vector_for_smoothing,vector_to_subtract_from=None, order=3, window_size=None, stype='sg'):
        nchan = vector_for_smoothing.shape[0]
        window_size = int(128/max(1,128*4/nchan)) if window_size is None else window_size
    ...
    def plot_source_bp_normalized_residuals(source_tsys, bandpass_tsys, model, frequencies, normalized_residuals, sigma,
                                            tsys_contamination_intervals, label_source='science',
                                            default_label = 'Tsys contamination', 
                                            default_color = 'red',first_plot_title='',level=None,
                                            background=None,
                                            other_emphasized_intervals = None, savefigfile=None, sci_spw_intervals=None):
            
        nchan = frequencies.shape[0]
        chans = np.arange(nchan)
        _nu_function = lambda ch: frequencies[0]+(frequencies[-1]-frequencies[0])/(nchan-1)*ch
        _channel_function = lambda nu: (nchan-1)/(frequencies[-1]-frequencies[0])*(nu-frequencies[0])
        ...
        for i in list(tsys_contamination_intervals):
            ...
            ax0.plot([_chsel[0],_chsel[-1]],source_tsys[[_chsel[0],_chsel[-1]]],'--', color='#1f77b4')
        ...
        ax0.plot(dif+_,label='difference', linewidth=3); # v2.5
        ax0.plot(model+_,'--',label='model')
        _min,_max = np.nanmin(np.r_[source_tsys,bandpass_tsys,dif+_]), np.nanmax(np.abs(np.r_[source_tsys,bandpass_tsys]))

        # _t = _min*.995-(trans_b)*(_min*.995-_max*1.005)
        #ax0.plot(chans,_t, label='ATM')
        ax0.set_ylim(_min-(_max-_min)/15,_max+(_max-_min)/15);
        ax0.set_title(first_plot_title);#ax0.set_xlabel('Frequency (MHz)');
        ax0.set_ylabel('Tsys (K)');
        
        
        put_legend_in_second_plot = False
        for i,interval in enumerate(list(tsys_contamination_intervals)):
            ax0.fill_betweenx(np.array([-2*_max,2*_max]),interval[0],interval[1],alpha=0.2,facecolor='red')
            ax1.fill_betweenx(np.array([-_max,2*np.nanmax(np.abs(normalized_residuals))]),interval[0],interval[1],alpha=0.2,facecolor='red', label=default_label if i==0 else None)
        ...
        for spw,spw_interval in sci_spw_intervals.items():
            ...
            ax0.add_patch(Rectangle((spw_interval[0],0.04),
                                    spw_interval[1]-spw_interval[0],0.01, fill=False, color='black', transform=trans))                
        ...
        for label, info in other_emphasized_intervals.items():
            ...
            if len(info['intervals'].flatten())==0: continue
            for i,interval in enumerate(list(info['intervals'])):
                color = info['color']
                color = 'grey' if color is None else color
                ax0.add_patch(Rectangle((interval[0],-0.1), interval[1]-interval[0],1.1, fill=True, color=color, 
                               alpha = alpha,  transform=trans, linewidth=0, hatch=hatch))
                
                trans1 = transforms.blended_transform_factory(ax1.transData, ax1.transAxes)
                ax1.add_patch(Rectangle((interval[0],-0.1), interval[1]-interval[0],1.1, fill=True, color=color, 
                               alpha = alpha,  transform=trans1, linewidth=0,label=label if i==0 else None, hatch=hatch))                  
                put_legend_in_second_plot = True
        ...
        ax0.legend(loc='upper right');ax0.grid();
        
        if put_legend_in_second_plot:
            ax1.legend();

        [x.secondary_xaxis('top', functions=(_nu_function,_channel_function)) for x in (ax0,ax1)]
        ax1_kelvin = ax1.secondary_yaxis('right', functions=(lambda nt: sigma*nt, lambda t: t/sigma),)
        ax1.set_ylabel('σ'); ax1_kelvin.set_ylabel('[K]')
        ax1.plot(chans,normalized_residuals,'.-');ax1.grid();
    ...
    def puff(intervals, delta=0, nchan=np.inf, chan_avoidance=None):
        ...
        for i in intervals:
            ...
            if chan_avoidance is not None:
                if np.any([p in range(int(pufflim0),int(i[0])+1) for p in chan_avoidance]):
                    pufflim0 = i[0]
                if np.any([p in range(int(i[1]),int(pufflim1+1)) for p in chan_avoidance]):
                    pufflim1 = i[1]
    ...
    def merge_intervals(intervals):
        ...
        if len(intervals.flatten())==0: return intervals 
    ...
    def complement_intervals(a): # This is the key function. Needs to work with infinities and empty sets.
...
def find_prominent_peaks_in_atm(n_sigma_atm, atm_model, temperature, sigma, frequencies=None):
    ...
    for p in atm_peak_centers:
        if np.all(np.abs(LINES_12C16O-frequencies[p])>np.abs(frequencies[0]-frequencies[1])):
            newpeaks.append(p)
    ...
    if np.any(_>n_sigma_atm*sigma/np.abs(temperature)):
        prominent_peaks_in_atm = atm_peak_centers[_ > n_sigma_atm*sigma/np.abs(temperature)]
...
def detect_tsys_contamination(bandpass,source,atm, n_sigma_atm_prominent_peak=7,n_sigma_detection_limit=7):
    ...
    source-=_lin_delta_s; bandpass-=_lin_delta_b
    ...
    t0,slope,base = basic_atm_asymmetric_fit(dif - np.nanmin(dif),1-atm, clip=4, iterations=3) # v3.3 set min of dif to zero. Include base.
    print(f't0 = {t0}, slope={slope}')
    nchan = len(dif)
    simplemm = mm = t0*atm + slope * np.linspace(-1,1,nchan)  + base + np.nanmin(dif)# v3.3
    channel_percentage_within_simple_model  = np.array([(100*np.sum(np.abs(dif  - simplemm)<=i*sigma_smooth)/nchan) for i in range(1,6,2)])
    print(f'channel percentage within n-sigmas: {channel_percentage_within_simple_model}%')
    
    # Identify prominent peaks in the atmospheric profile. Ignore CO as a prominent peak.
    # plt.plot(source,'.-');#plt.plot(dif);print(sigma_smooth);input()
    prominent_peaks_in_atm = find_prominent_peaks_in_atm(atm_model = atm,# frequencies = freqs_b, 
                                                        temperature = t0, sigma=sigma_smooth,
    ...
    for k in range(2):
        if (prominent_peaks_in_atm is not None or np.any(channel_percentage_within_simple_model < np.array([20,40,50]))):
            print("Not simple")
            dega = 3 if prominent_peaks_in_atm is not None else 0 # v3.5
            dega += k
            deg_baseline = 3
            xp_mod, baseline_mod = model_break(atm,prominent_peaks_in_atm, window_length = 5*int(nchan/128))

            _dif = dif.copy()
            for i in range(2):
                chi2 = lambda var: np.sum(no_nans((_dif-modified_atmospheric_profile(model=atm, degree_baseline=deg_baseline,
                                                            baseline=baseline_mod, scales=list(range(1,dega)),
                                                            coefficients = var, segments=xp_mod)))**2/sigma_smooth**2) 
                var0 = list(np.r_[t0,np.zeros(deg_baseline+1),np.zeros(2*dega*xp_mod.shape[0])])
                var0[2] = slope
                ...
                baseline, modline = modified_atmospheric_profile(model = atm,  degree_baseline=deg_baseline,
                                                            baseline=baseline_mod,scales=list(range(1,dega)), 
                                                            coefficients = res['x'], segments=xp_mod[0:], split=True)
                mm = baseline+modline
        ...
        rescale_smooth_sigma_factor = max(smoother_sigma(_normres,stype='constant'),0.5)
        if np.any(normres[prominent_peaks_in_atm]>=n_sigma_detection_limit):
            #print("**",normres[prominent_peaks_in_atm])
            print(f"detect_tsys_contamination: re-iterating ATM fitting with higher order.")
        else:
            break
   
    # print(np.nanmedian(np.abs(dif-mm-np.nanmedian(dif-mm))))
    
    prominent_peaks_in_atm = [] if prominent_peaks_in_atm is None else prominent_peaks_in_atm
    ...
    pcline_peak_chans, pcline_intervals = refitting_intervals_bp_to_source(source=source, bp=bandpass, sigma=sigma, normalized_data = normres, 
                                                                   detection_limit = n_sigma_detection_limit,
                                                                    base_detection=(1-2)/(4096-128)*(nchan-128)+2, verbose=True)

    #plt.plot(dif);plt.plot(mm);input()
    return {'contamination_peaks':pcline_peak_chans, 'contamination_intervals':pcline_intervals, 
            'normalized_residuals': normres, 'sigma':sigma, 'prominent_atmospheric_peaks_location':prominent_peaks_in_atm,
...
def co_telluric_intervals(normalized_spectrum,frequencies_mhz):
    ...
    nu0,nu1,dnu = np.min(frequencies_mhz), np.max(frequencies_mhz), np.mean(np.abs(frequencies_mhz[1:]-frequencies_mhz[0:-1]))
    line_names = []
    line_frequencies = []
    ...
    if len(line_names)<=0: return results
    line_frequencies = np.array(line_frequencies)
    peaks,prominence_dictionary = find_peaks(normalized_spectrum,prominence=5)
    selection = np.array([np.min(np.abs(frequencies_mhz[p]-line_frequencies)) < 2*dnu for p in peaks])
    if len(selection)<=0: return results
    ...
    if len(peaks)<=0: return results
    widths, heights, left_ipss, right_ipss = peak_widths(normalized_spectrum,peaks,rel_height=0.5)
    # assert False, peaks
    results = {'co_lines':[],'co_line_frequencies':[],'ranges':[]}
    for p,width, height, left_ips, right_ips in zip(peaks,widths, heights, left_ipss, right_ipss):
        ...
        for k,v in CO_LINES_MHZ.items():
            if np.abs(frequency_of_peak - v)<2*dnu:
                ...
                results['ranges'].append([max(0,p-width/2.35*3),min(nchan-1,p+width/2.35*3)])
                break        
    results['ranges'] = np.array(results['ranges'],dtype=int)
    return results


def get_tsys_contaminated_intervals(tsysdata, spwlist = None , fieldlist=None,
large_interval_warning_limit = 0.25, # % of spectral window covered by line for warning
n_sigma_atm_prominent_peak = 7, plot=False, savefigfile=None, remove_n_extreme=1, relative_detection_factor=0.05, # v3.1
n_sigma_detection_limit=7): 
    ...
    if len(spwlist)==0: print("get_tsys_contaminated_intervals. Warning: no spwlist -> no output.")
    _ = set(list(tsys.tsysdata[tsys.tsysfields.index('field')]))
    if fieldlist is None:
        fieldlist = list(_)
    fieldlist = list(_.intersection(set(fieldlist)))
    if len(fieldlist)==0: print("get_tsys_contaminated_intervals. Warning: no fieldlist -> no output.")
    with warnings.catch_warnings():
        warnings.filterwarnings(action='ignore', message='Mean of empty slice')
        average_difference, average_science, average_bp, average_antenna = tsys.filter_statistics_and_dif(
                                                                                statistics = np.nanmean,  
                                                                                remove_n_extreme = remove_n_extreme)
        min_difference, min_science, min_bp, _ = tsys.filter_statistics_and_dif(statistics = np.nanmin,
                                                                                remove_n_extreme = remove_n_extreme)

    spws, intents, scans, fields = (tsys.tsysdata[tsys.tsysfields.index(f)] for f in ['spw','intent', 'scan', 'field'])
    _ = []
    ...
    all_absorption_curves = tsys.absorptiondata[tsys.absorptionfields.index('absorption')]
    ...
    for spw,field in itertools.product(spwlist,fieldlist):
        if intent_dict[field] == 'bandpass': continue
        key = f'{spw}_{field}'
        print(f'-- {key}')
        _ = set(scans[(intents=='bandpass')*(spws==spw)].tolist())
        assert len(_)==1, "get_tsys_contaminated_intervals: more than 1 BP scan per spw in EB!"
        scan_b = _.pop()

        freqs_mhz = all_freqs_mhz[np.nonzero(tsys.specdata[tsys.specfields.index('spw')]==spw)[0][0]]
        freqs_b = freqs_mhz ###
        ...
        _ = detect_tsys_contamination(bandpass=spec_b,source=spec_s,atm=1-trans_b,n_sigma_detection_limit = n_sigma_detection_limit)
        ...
        sigma_mad_dif = median_abs_deviation(dif-mm, scale='normal', nan_policy='ignore')
        print("large_baseline_residual *** \n",f"\tsigma_mad_dif,sigma_smooth_bp,sigma_smooth_s,prominent_peaks_in_atm = {sigma_mad_dif:.2g},{sigma_smooth_bp:.2g},{sigma_smooth_s:.2g},{prominent_peaks_in_atm}")
        print(f"\tchans over 5sigma = {np.sum((dif-mm)>5*sigma_smooth_bp)}, nchan={nchan}, nchan/4={nchan/4}")
        if False:
            plt.close();plt.plot((dif-mm)/sigma_smooth_bp);
            input()        
        if (sigma_mad_dif>3*sigma_smooth_bp
            and np.sum((dif-mm)>5*sigma_smooth_bp,dtype=int)>nchan/4
            and len(prominent_peaks_in_atm)==0):# v3.4
        ...
        speaks, prominence_dictionary_source_peaks =  find_peaks(spec_s/sigma_smooth_s ,prominence = n_sigma_detection_limit*((1-0.)/1920*(nchan-128)+0.))        
        speaks = width_fit_filter(data=spec_s/sigma_smooth_s,peaks = speaks, fwhm_limit =(150-5)/1920*(nchan-128)+5, window = int(9+ (200-9)/1920*(nchan-128))) 
        common_peaks1, common_peaks2, more_info_common_peaks = find_repeated_peaks(data1=spec_s/sigma_smooth_s, data2=spec_b/sigma_smooth_bp, prominence=n_sigma_detection_limit/1.5, 
                                       width=int(nchan/64))#
        # assert False, more_info_common_peaks
        # change above difference_tolerance_ratio treatment in v3.3 
        delta_tsys_detection_level = relative_detection_factor*np.nanmedian(min_science[key][corr]) # v3.1
        print(f"common_peaks1={common_peaks1}, common_peaks2={common_peaks2}",
        ...
        concommitant_peak_max_distance = ((4-2)/2048*(nchan-128)+2.)
        for pcli, pclp in zip(list(pcline_intervals), list(pcline_peak_chans)):
            ...
            delta_dif = np.ptp(spec_s[pcli[0]:pcli[1]+1] - spec_b[pcli[0]:pcli[1]+1])
            ...
            telluric_channel_tolerance = int((32-1)/(4096-128)*(nchan-128)+1)
            telluric_channels = np.r_[[common_peaks1 
                +_ for _ in range(-telluric_channel_tolerance,telluric_channel_tolerance+1)]].flatten()
            # assert False, (pclp in telluric_channels, pclp, telluric_channels, interval_comparison)
            #concommitant_peak = 
            if (pclp in telluric_channels
                and interval_comparison>interval_range_limit):  # v3.1
                if np.any(np.abs(freqs_b[int(pclp)]-LINES_12C16O)<2*chansize_mhz):        
                    if interval_comparison<0.7:
                        warnings_list.append(["YELLOW Warning: large difference between bandpass and field telluric CO line.", 
                ...
                elif np.any(np.array([np.prod(pcli-_) for _ in prominent_peaks_in_atm])<=0):
                    atm_residual_intervals.append(pcli)
                else:
                    common_feature_intervals.append(pcli)
            elif np.any(np.array([np.prod(pcli-_) for _ in prominent_peaks_in_atm])<=0): # v2.3
                atm_residual_intervals.append(pcli)
                ...
                gaussian_check,gaussian_plus_baseline_model = gaussian_line_fit(x=np.arange(pcli[0],pcli[1]+1),
                ...
                if (gaussian_check[0]< n_sigma_detection_limit*((1-0.5)/(4096-128)*(nchan-128)+0.5) 
                        or not np.any((pcli[1]-pcli[0]>_auxipeak)*(_auxipeak > 0))):
                    print(f"{pcli} -> possible (failed concommitant peak gaussian check)")
        ...
        possible_line_intervals = difference_intervals(np.array(possible_line_intervals),line_intervals)
        # telluric_intervals = difference_intervals(np.array(telluric_intervals),line_intervals) # commented in v3.3
        telluric_intervals = np.stack(telluric_intervals, axis=0) if len(telluric_intervals)>0 else np.array([])# v3.3
        atm_residual_intervals = difference_intervals(np.array(atm_residual_intervals),line_intervals)
        common_feature_intervals = difference_intervals(np.array(common_feature_intervals),line_intervals)
        low_tsys_contamination =  difference_intervals(np.array(low_tsys_contamination),line_intervals)
        
        
        line_intervals = puff(line_intervals, delta=int((16-0)/(4096-128)*(nchan-128)+0), 
                            nchan=nchan, ### v2.2
                            chan_avoidance = common_peaks1[peak_prominences(spec_b,common_peaks1)[0] > 
                                        n_sigma_detection_limit*sigma_smooth_bp] )  ### v3.3
        
        labeling, number_of_intervals = label(normres>n_sigma_detection_limit)
        if number_of_intervals>0:
            auxiliary_intervals = np.array([[min(chans[labeling==l]),max(chans[labeling==l])+1]for l in range(1,number_of_intervals+1)])
            auxiliary_intervals = np.array([interval for interval in auxiliary_intervals if len(intersection_intervals(np.array([interval]),line_intervals))>0])
            line_intervals = union_intervals(line_intervals,auxiliary_intervals)
        line_intervals = difference_intervals(line_intervals, np.array(telluric_intervals)) # v3.3
        if len(co_telluric)>0: line_intervals = difference_intervals(line_intervals, co_telluric['ranges']) # v3.5
        ...
        interval_dict = {'telluric_intervals':telluric_intervals,
        ...
        for i in line_intervals:
            ...
            wide = False
            velocity_width_kms = abs(i[1]-i[0])*relative_channel_width*scipy.constants.c/1e3
            dv_criterion = velocity_width_kms>=MAX_VELOCITY_WIDTH
            spw_percentage_criterion = abs(i[1]-i[0])> nchan*large_interval_warning_limit 
            if spw_percentage_criterion or dv_criterion: # based on Dame's CO 1-0 map.
                ...
                message = ' '.join([f"Channel range {int(i[0])}~{int(i[1])} equivalent to",
                    f"{velocity_width_kms:.3g} km/s > {MAX_VELOCITY_WIDTH} km/s" if dv_criterion else f"{int(abs(i[1]-i[0])/nchan*100)}% of the spw",
                    f"in spw{spw} and field={field}."])
                                
                warnings_list.append([warning,message])
            print(f"Range {i} equivalent to {velocity_width_kms} km/s or {int(abs(i[1]-i[0])/nchan*100)}% of the SpW.")
            if not wide:
                _.append(i)
        ...
        if plot:
            if large_baseline_residual:  print(f"{key} large_baseline_residual: {large_baseline_residual}")
            plot_source_bp_normalized_residuals(source_tsys=spec_s, bandpass_tsys=spec_b, 
                model=mm, frequencies=freqs_b/1e3, sigma=np.nanmedian(sigma), # freqs in GHz. sigma. v3.1 
                #level=(None,n_sigma_detection_limit),
                normalized_residuals = normres, tsys_contamination_intervals=merge_intervals(line_intervals), 
                ...
                savefigfile = (None if savefigfile is None else f'{savefigfile}_spw{spw}_field{field}.png'),
                label_source = intent_dict[field],
                other_emphasized_intervals = {'Tsys contamination?':{'color':'grey','intervals':merge_intervals(possible_line_intervals)},
                                              'Telluric':{'color':'blue','intervals':merge_intervals(telluric_intervals)},
                                              'Off science':{'color':'aquamarine','alpha':0.4,'intervals':merge_intervals(off_science_line_intervals)},
                                              'ATM residual':{'color':'#6a4530','intervals':merge_intervals(atm_residual_intervals)},
                                              'Common BP feature':{'color':'#6a4530','intervals':merge_intervals(common_feature_intervals)},
                                              f'dT/T<{relative_detection_factor*100:.1f}%':{'color':'purple','hatch':'/','intervals':low_tsys_contamination},},
                sci_spw_intervals = sci_spw_intervals_in_tsys_channels,)

        result[key] = {'possible_tsys_contamination':merge_intervals(possible_line_intervals),
                        ...
                        'tsys_contamination_offscience': merge_intervals(off_science_line_intervals),}                           
...
def separate_non_intersecting_sci_spw(tsysdata, chan_tsys_intervals, tsysspw): # v2.4
    ...
    scispws_intervals = np.array([np.sort(tsysdata.msspec[spw][0](np.array([0,tsysdata.msspec[spw][-1]-1]))) for spw in scispws])
    ...
    for i in intervals:
        if np.prod(intersection_intervals(i[np.newaxis,:],puffedscispws_intervals).shape)<=0:
            non_intersect.append(i)
        else:
            intersect.append(i)
    return {'in':np.array(intersect), 'off':np.array(non_intersect),'sci_intervals':dict(((spw,scispws_intervals[i]) for i,spw in enumerate(scispws)))}



def intervals_to_casa_string(intervals, scaled_array=None, unit='', format=".0f")->str:
    ...
    for k in range(len(np.sort(intervals,axis=0))):
        ...
        if scaled_array[int(l0)]>scaled_array[int(l1)]: 
            l0,l1 = l1,l0 # v3.4
        rs += f"{scaled_array[int(l0)]:{format}}~{scaled_array[int(l1)]:{format}}{unit}" if l0!=l1 else f"{scaled_array[int(l0)]:{format}}{unit}" #l0!=l1 v3.4
        i+=1
    return rs
...
def make_tsyscontamination_html_log(title="TsysCont"):
    ...
    if len(txtfiles)==0: return False
    logdirname = "tsys_contamination_log"
    if os.path.exists(logdirname): os.system(f"rm -rf {logdirname}")
...
"""
    ...
    my_html.write(f'<title>{title}</title>\n<link href="txtstyle.css" rel="stylesheet" type="text/css" />\n')
    ...
    for txtfile in txtfiles:
        ...
        pngs = glob.glob(f"{msfile}_tsys_contamination_spw*_field*.png"); pngs.sort()         
        for pngfile in pngs:
...
def main():
    ...
    tsystables = testtables if 'testing' in VERSION else glob.glob("uid___*.ms.h_tsyscal.*.tsyscal.tbl")
    #random.shuffle(tsystables)
    for tsystable in tsystables[0:]:
        tsys= TsysData(tsystable=tsystable, load_pickle=True, single_polarization=(tsystable=="uid___A002_X10ac6bc_Xc408.ms.h_tsyscal.s6_1.tsyscal.tbl"))
        ...
        print(("-"*40)+f"\n{tsys.msfile}\n"+("-"*40)) # v2.3        
        line_contamination_intervals, warnings_list = get_tsys_contaminated_intervals(tsys, plot= True, # large baseline residual v3.3
                    #spwlist=[np.int(30)],fieldlist=[np.int64(2)],# this selection does not work with the saved spool sample
                    remove_n_extreme=2,relative_detection_factor=0.5/100,savefigfile=f'{tsys.msfile}_tsys_contamination')
        # assert False, ""
        for k,v in line_contamination_intervals.copy().items():
            if np.sum(np.array([len(vv) for kk,vv in v.items()])) == 0: del(line_contamination_intervals[k])
        ...
        pl_run_dir = ''
        m = re.match(r'.*([0-9]{4}\..\.[0-9]{5}\.[^_]+_[0-9]{4}_[0-9]{2}_[0-9]{2}T[0-9]{2}_[0-9]{2}_[0-9]{2}\.[0-9]+).*',os.getcwd())
        if m:
        ...
        for field in field_contamination:
            ...
            for spw in field_contamination[field]:
                if field_intent_dict[field] == 'bandpass': continue
                key = f'{spw}_{field}'
                spw = np.int64(spw)
                freqs_ghz = all_freqs_mhz[np.nonzero(tsys.specdata[tsys.specfields.index('spw')]==spw)[0][0]]/1000
                rs = intervals_to_casa_string(line_contamination_intervals[key]['tsys_contamination'])
                rsf = intervals_to_casa_string(line_contamination_intervals[key]['tsys_contamination'], scaled_array = freqs_ghz, unit='GHz', format=".3f")
                if rs!='':
                    spw_ranges.append(f'{spw}:{rs}')
                    spw_ranges_freq.append(f'{spw}:{rsf}')
            
            if len(spw_ranges)==0: continue #v2.2
            spw_ranges = ','.join(spw_ranges)
            spw_ranges_freq = ','.join(spw_ranges_freq)
            contamination_scans = field_scanlist_dict[field]; contamination_scans.sort()
