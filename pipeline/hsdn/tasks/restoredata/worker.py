...
class SDAmpCalWorkerInputs(vdp.StandardInputs):
...
class SDAmpCalWorker(basetask.StandardTaskTemplate):
    """
    Per MS caltable creation
    """
    Inputs = SDAmpCalWorkerInputs    
    ...
    def prepare(self):
        ...
        if not os.path.exists(vis):
            ...
            return SDAmpCalWorkerResults(os.path.basename(vis))
        vis = os.path.basename(vis)
        if vis not in factors:
            return SDAmpCalWorkerResults(vis)
        ...
        origin = callibrary.CalAppOrigin(task=SDAmpCalWorker,
        ...
        return SDAmpCalWorkerResults(vis, calapp=calapp, factors=factors_used)
    ...
    def analyse(self, result):
        ...
        if len(result.ms_factors) == 0:
            ...
            LOG.warning("No factor data is given for MS '%s'" % name)
            return result
        ...
        for spw in ms.get_spectral_windows(science_windows_only=True):
            ...
            if spwid not in result.ms_factors:
                result.factors_ok = False
                LOG.warning("No factor data is given for Spw=%d of %s" % (spwid, name))
                continue
            ...
            for ant in ms.get_antenna():
                ...
                if is_anonymous_ant:
                    ...
                    result.factors_ok = False
                    LOG.warning("No factor data is given for Spw=%d, Ant=%s of %s" % (spwid, ant_name, name))
                    continue
                ...
                for pol in pol_list:
                    ...
                    if not ok:
                        LOG.warning("No factor data is given for Spw=%d, Ant=%s, Pol=%s of %s" %
