    ...
    def handle(self, context: 'Context', result: 'SDApplycalResults') -> None:
        ...
        for spw in spwids:
            ...
            if is_figfile_exists:
                ...
                longmsg = 'Generating amp vs time plot for spw{0} and ant=all of {1} was successful.'.format(spw, vis)
            else:
                shortmsg = 'Generating amp vs time plot was failed.'
                longmsg = 'Generating amp vs time plot for spw{0} and ant=all of {1} was failed.'.format(spw, vis)
                score = 0.65
            ...
            for ant in ants:
                ...
                if is_figfile_exists:
                    shortmsg = 'Generating amp vs time plot was successful.'
                    longmsg = 'Generating amp vs time plot for spw{0} and {1} of {2} was successful.'.format(spw, ant, vis)
                else:
                    shortmsg = 'Generating amp vs time plot was failed.'
                    longmsg = 'Generating amp vs time plot for spw{0} and {1} of {2} was failed.'.format(spw, ant, vis)
                    score = 0.65
                scores = [pqa.QAScore(score, longmsg=longmsg, shortmsg=shortmsg)]
                result.qa.pool.extend(scores)
