...
class SDInspection(object):
    ...
    def execute(self) -> Tuple[Dict[int, MSReductionGroupDesc], Dict[str, Union[str, Dict]]]:
        ...
        return reduction_group, org_directions, msglist, grouping_result['RASTERHEURISTICS']
...
#         row = numpy.asarray(datatable.getcol('ROW'))
        ...
        ms = self.ms
        raster_heuristics_result = RasterScanHeuristicResult.generate(ms, RasterScanHeuristicResult.IMPORTDATA)
...
#                     row_sel = numpy.take(row, id_list)
                    ...
                    if pattern == 'RASTER' and self.hm_rasterscan == 'direction':
                        ...
                        try:
                            ...
                            _log_dict = {'ANTENNA': ms.antennas[ant].name,
                                         'EB': ms.execblock_id}
                            merge_table, merge_gap = raster_heuristic(sra_sel, sdec_sel, _log_dict, raster_heuristics_result)
        ...
        grouping_result['RASTERHEURISTICS'] = raster_heuristics_result
