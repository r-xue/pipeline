...
%if result[0].inputs['statwtmode'] == 'VLA' :
<table style="float: left; margin:0 10px; width: auto; text-align:center" class="table table-bordered table-striped ">
	<caption>Summary of before/after-statwt antenna-based weights (<i>W</i><sub>i</sub>) for each scan. The antenna-based weights are derived from the visibility WEIGHT column: <i>W</i><sub>ij</sub>&asymp;<i>W</i><sub>i</sub><i>W</i><sub>j</sub>.
    </caption>
	<thead>
		<tr>
	...
	<tbody>
		...
		<tr>
            ...
            <td>${format_wt(before_by_scan[i]['med'])}</td>
            % if before_by_scan[i]['quartiles'] is not None:
                <td>${format_wt(before_by_scan[i]['q1'])}/${format_wt(before_by_scan[i]['q3'])}</td>
            % else:
                <td>N/A</td>
            % endif            
            <td>${format_wt(before_by_scan[i]['mean'])} &#177 ${format_wt(before_by_scan[i]['stdev'])}</td>         
