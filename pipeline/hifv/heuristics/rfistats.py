...
def plot_sefd(sefd, spws_per_band=None):

    fig, ax = plt.subplots(figsize=(8, 4))

    for band, sedf_per_band in sefd.items():
...
def correct_rflag_ftdev(rflag_report, bbspws, spw_rms_scale=None):
    """dervive the corrected freqdev/timedev for applying rflag 
...
def vla_basebands(ms, science_windows_only=True):
    """get spw from each VLA baseband 

    Args:
        ms: a MeasurementSet domain object

    - originally located in circfeedpolcal.py/checkflag.py from BK (refactoring candidates)
    - adopted into hifv.heuristics.rfi for PIPE-685/987
    """
    ...
    return vlabasebands, banddict


def get_spw_rms_scale(ms, sefd_database, science_windows_only=True):
    """[summary]
    ...
    for spw in ms.get_spectral_windows(science_windows_only=science_windows_only):
        ...
        try:
            band = spw.name.split('#')[0].split('_')[1]
            baseband = spw.name.split('#')[1]
            sedf_per_band = sefd_database[band]
            ...
            if frequency_mhz < min(sedf_per_band[:, 0]) or frequency_mhz > max(sedf_per_band[:, 0]):
               LOG.warn("The mean frequency of spw {!s} is out of the SEFD database coverage.".format(spw.id))
        except Exception as ex:
            LOG.warn("Exception: Baseband name cannot be parsed.{!s}".format(str(ex)))
