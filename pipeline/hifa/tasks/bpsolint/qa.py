...
class BpSolintQAHandler(pqa.QAPlugin):
    ...
    def handle(self, context, result):
        ...
        score1 = self._missing_phaseup_snrs(ms, result.spwids, result.phsolints)
        score2 = self._missing_bandpass_snrs (ms, result.spwids, result.bpsolints)
        score3 = self._poor_phaseup_solutions(ms, result.spwids, result.nphsolutions, minphaseupints)
        score4 = self._poor_bandpass_solutions(ms, result.spwids, result.nbpsolutions, minbpnchans)
    ...
    def _poor_phaseup_solutions(self, ms, spwids, nphsolutions, min_nsolutions):
        ...
        """
        return qacalc.score_poor_phaseup_solutions(ms, spwids, nphsolutions, min_nsolutions)
    ...
    def _poor_bandpass_solutions(self, ms, spwids, nbpsolutions, min_nsolutions):
        ...
        minimum number of solutions
        """
        return qacalc.score_poor_bandpass_solutions(ms, spwids, nbpsolutions, min_nsolutions)
