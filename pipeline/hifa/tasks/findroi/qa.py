...
class FindROIQAHandler(pqa.QAPlugin):
    ...
    def handle(self, context, result):
        if result.errors and result.summary.get('n_source_spws', 0) == 0:
            score = pqa.QAScore(0.0, longmsg='hifa_findroi did not produce usable results.', shortmsg='findROI failed')
        elif result.errors:
            ...
            score = pqa.QAScore(
                score_value,
                longmsg=f'hifa_findroi completed with {n_failed_spws} failed SPW task(s); inspect the weblog error list for details.',
                shortmsg='findROI partial failure',
            )
        elif result.summary.get('n_source_spws', 0) == 0:
            score = pqa.QAScore(0.0, longmsg='hifa_findroi found no source/SPW products.', shortmsg='No findROI products')
        else:
            ...
            score = pqa.QAScore(
                score_value,
                longmsg=f'hifa_findroi produced continuum selections for {n_cont} of {n_total} source/SPW products.',
