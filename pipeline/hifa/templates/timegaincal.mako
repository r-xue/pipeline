...
<%self:plot_group plot_dict="${diagnostic_phase_vs_time_plots}"
	...
	<%def name="preamble()">
		<p>These diagnostic plots show the phase solution for a calibration
            generated using a short solution interval (in the case of the
            checksource(s), they are taken from the tables generated by
            hifa_gfluxscale). In case of very low SNR on a particular phase
            calibrator or check source (if present), solutions are averaged in
            time with a solint as reported for that field in the 'hifa_spwphaseup'
            stage. This calibration is not applied to the target. One plot is
            shown for each non-combined spectral window, with phase correction
            plotted per antenna and correlation as a function of time.</p>
