...
```
basestring, except, execfile, filter, has_key, import, map, ne, print, raise,
reduce, repr, tuple_params, ws_comma (optional), zip
```

Rules that can be updated on trunk during C7 dev:

```
idioms, set_literal (optional), types (after idioms)
```
